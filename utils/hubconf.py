...
from utils.general import check_requirements, set_logging,check_img_size
...
@st.cache_resource
def loadModel(path_or_model='path/to/model.pt', gpu=False):
    # Load model
    device = select_device('0' if gpu else 'cpu')
    model = attempt_load(path_or_model, map_location=device)  # load FP32 model
    return model

...
def get_user_model():
    ...
    if model_src == "file upload":
        model_bytes = st.sidebar.file_uploader(
            "Upload a model file", type=['pt'])
        if model_bytes:
...
def main():
    ...
    # upload model
    model_src = st.sidebar.radio("Select yolov5 weight file", [
    ...
    if not os.path.isfile(cfg_model_path):
        ...
        if torch.cuda.is_available():
            device_option = st.sidebar.radio(
                "Select Device", ['cpu', 'cuda'], disabled=False, index=0)
        else:
            device_option = st.sidebar.radio(
                "Select Device", ['cpu', 'cuda'], disabled=True, index=0)
